  ...
  return (
    <div className="min-h-screen w-full flex flex-col gap-4">
      ...
      <div className="flex justify-start gap-4 items-center w-full overflow-x-auto">
        <button
          ...
          className={`px-4 py-2  text-white font-semibold ${
            isPublished
              ...
              : "bg-cyan-600 text-cyan-50 hover:bg-cyan-700"
          } transition-colors`}
        >
          {isPendingPage ? (
            <>
        ...
        <button
          onClick={() =>
            router.push(`/pages/${pageData.project.slug}/${pageData.slug}/edit`)
          }
          className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-teal-50 font-semibold  transition-colors"
        >
          Edit
        </button>
        ...
        <ShareButton
          ...
          showDefault={
            <span className="text-white p-2 bg-cyan-600 whitespace-nowrap">
              Share Now
            </span>
          }
        />
        ...
        <button
          ...
          onClick={() => handleDeletePage(pageData.id)}
          ...
          {pageData.isPublished && (
            <div className=" whitespace-nowrap">
              <div className="border px-2 p-1  bg-slate-200 text-slate-700 w-fit overflow-x-auto whitespace-nowrap flex items-center">
                <Link
                  href={`/${pageData.user.username}/${pageData.project.slug}/${pageData.slug}`}
                  target="_blank"
                  className="mr-2 text-slate-600 hover:text-slate-800 font-bold flex items-center gap-1"
                >
                ...
                <Link
                  target="_blank"
                  href={`/${pageData.user.username}/${pageData.project.slug}/${pageData.slug}`}
                  className="inline hover:underline max-w-32 truncate"
                >
