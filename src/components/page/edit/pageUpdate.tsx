...
import { PageEditSchema, ProjectCreatorSchema } from '@/schemas';
import { z } from 'zod';
import { editPage } from '@/actions/user/manage-page';
import { createProject } from '@/actions/user/manage-project';
import { DialogDescription } from '@radix-ui/react-dialog';
import { useRouter } from 'next/navigation';
import { PageWithProjectType } from '@/lib/data';

export default function PageUpdate({projects, pageData}:{projects:any, pageData:PageWithProjectType}) {
  
  const router = useRouter()

  // State Management

  const [selectedProject, setSelectedProject] = useState(pageData.project);
  const [projectName, setProjectName] = useState(selectedProject.name);
  const [projectSlug, setProjectSlug] = useState(selectedProject.slug);
  const [pageName, setPageName] = useState(pageData?.name || '');
  const [pageSlug, setPageSlug] = useState( pageData?.slug || '');
  ...
  const genBtnVisible = 
  !(!(selectedProject?.name || pageData?.project.name) || 
  !(selectedProject?.slug || pageData?.project.slug) || 
  !(pageName || pageData?.name) || 
  !(pageSlug || pageData?.slug )|| 
  !(pageDesc || pageData?.content));
  ...
  const onSubmitPage = (values: z.infer<typeof PageEditSchema>) => {
    setError("");
    setSuccess("");
    startPageTransition(() => {
        editPage(pageData.id, values)
            .then((data) => {
                if (data.error) {
                ...
                if (data.success) {
                    ...
                    router.push(`/pages/${projectSlug}/${data.updatedPage.slug}`)
                }
            })
...
const handleSavePage = () => {
    ...
    <div className="w-full mx-auto flex flex-col gap-4">
      ...
      <div className="w-full flex gap-4 overflow-x-scroll flex-nowrap items-start">
        ...
        <Select 
          ...
          {((projects && (projects.length > 0)) || pageData?.project.slug )? (
  ...
  <div className="flex flex-nowrap gap-4 ">
    <div className=" flex gap-2 items-center justify-start whitespace-nowrap px-3 p-1 bg-green-100 text-green-600 ">
      ...
      <p className="text-sm truncate ">{projectName}</p>
    </div>
    <div className=" flex gap-2 items-center justify-start whitespace-nowrap px-3 p-1 bg-indigo-100 text-indigo-600 ">
      <h3 className="font-semibold  ">Project Slug:</h3>
      <p className="text-sm truncate ">{projectSlug}</p>
    </div>
